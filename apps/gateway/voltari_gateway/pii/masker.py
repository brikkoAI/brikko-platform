...
from voltari_gateway.pii.checksums import inn10, inn12, luhn, ogrn, ogrnip, snils
...
def _collect_detections(
    ...
    from voltari_gateway.pii.ru_person import find_persons

    for person in find_persons(text):
        span = (person.start, person.end)
        if _overlaps(span):
            continue
        detections.append((person.start, person.end, "NAME", person.surface, person.normalized))
        consumed.append(span)
