...
* A *predicted span* comes from
  :func:`voltari_gateway.pii.masker._collect_detections` — same shape
  ``(start, end, category)``.
...
def _detect_spans(text: str) -> set[SpanKey]:
    ...
    """
    from voltari_gateway.pii.masker import _collect_detections
...
def run_benchmark(corpus_path: Path, out_dir: Path) -> dict[str, CategoryStats]:
    ...
    from voltari_gateway.pii.ru_person import warm_up
