...
These tests describe the contract of :pyfunc:`voltari_gateway.pii.ru_person.find_persons`:
...
from voltari_gateway.pii.ru_person import PersonSpan, find_persons
...
class TestSpansAndOffsets:
    ...
    def test_no_overlapping_spans(self) -> None:
        ...
        for i in range(len(spans) - 1):
            assert spans[i].end <= spans[i + 1].start, f"overlap: {spans[i]} vs {spans[i + 1]}"
